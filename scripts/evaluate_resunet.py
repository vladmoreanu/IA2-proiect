...
from modeling.metrics import PSNR, MSE, Combined_L1_SSIM
...
CONFIG_PATH = Path("./results/ResUNet/ds_06/conf-2026-06-15_21-01.json").resolve()
...
def main():
    ...
    time = "2026-06-15_21-01" or datetime.now().strftime("%Y-%m-%d_%H-%M")
    ...
    train_loader = DataLoader(
        Subset(dataset, train_idx), shuffle=True, **run_config.dataloader
    )
    val_loader = DataLoader(
        Subset(dataset, val_idx), shuffle=False, **run_config.dataloader
    )
